...
#H0 = tensor(ini_mats(N),ini_matp(pcut,N))
...
plt.grid()
#%%
dresult = result.expect[0]+result.expect[1] - (result1.expect[0]+result1.expect[1])
#plt.plot(times,dresult)
#compute the detunings 
wlist3 = np.array(Transfreq(N,fa,ft))* fa * 1000 #this is used to compute deltam in kHz
mu = (1000*ft + delta) #kHz
dlist = (mu-wlist3) #kHz  
#%%
label1 = 'detuning ' + str(dlist[0]) + ' kHz'
label2 = 'detuning ' + str(np.round(dlist[1])) + ' kHz'
from scipy import signal
import matplotlib.pyplot as plt
f, Pxx_den = signal.periodogram(dresult, 10000)
plt.semilogy(f, Pxx_den,)
plt.plot([dlist[0],dlist[0]],[0,1],label=label1)
plt.plot([dlist[1],dlist[1]],[0,1],label=label2)
#plt.ylim([1e-7, 1e2])
plt.xlabel('frequency [kHz]')
plt.ylabel('PSD [V**2/kHz]')
plt.title(title)
plt.ylim(10**(-8),1)
plt.xlim(100,150)
plt.legend()
