...
'''
elist = [sp_op.spin_measure(ion_sys,['z0']),
         sp_op.phonon_measure(ion_sys,df_p, mindex=1)#tilt mode population
]
#solve time evolution for a single energy splitting
H0  = etrans.H_res(ion_sys,laser1, Omegax=omegax,Omegay=0,Omegaz=deltaE,i_type=0)#i_type=0->zz, i_type=1->laser phase (0 for x, pi/2 for y)
clist1 = disp.cooling(ion_sys,gamma_list = [0.05*np.abs(delta)/(2*np.pi),0],
                      nbar_list = [0.01],df=1, normalized = False)
rho0 = sp_op.rho_thermal(ion_sys, nbar_list=[[0.01]],s_config=['z0'],ket = False)
