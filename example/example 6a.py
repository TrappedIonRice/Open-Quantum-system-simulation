...
ion1.laser_couple = [0]
ion1.pcut = [[5,5],[5,5]]
ion1.active_phonon = [[0,1],[1,2]] #only consider com, tilt for axial and tilt rock for radial
ion1.list_para()
ion1.plot_freq()
#%%set up ion2, has same Hilbert space as ion1 but have laser couple to Axial tilt 
ion2 = copy.copy(ion1)
ion2.df_laser = 0
ion2.delta = 2* ion1.delta
ion2.delta_ref = 1
#ion2.delta = ion1.delta
ion2.list_para()
ion2.plot_freq()
#%%
Omegax = 0.25*np.abs(ion1.delta)
deltaE = 0.25*ion1.delta
'''
time scale
'''
t_scale = np.abs(ion1.delta)
tplot = np.linspace(0,100,10000)
times = tplot*2*np.pi/t_scale
'''
measure axial tilt mode and radial rock mode population
'''
ket0 = exop.ini_state(ion1,[0],[[0,0],[0,0]],1)
op1 = exop.phonon_measure(ion1,0,0) #axial com up
op2 = exop.phonon_measure(ion1,1,0) #axial tilt up
op3 = exop.phonon_measure(ion1,0) #radial tilt up
op4 = exop.phonon_measure(ion1,1) #radial rock up 
...
operator_a = exop.p_ladder(ion1,1,1,0) #creation operator on tilt axial mode
operator_b =  exop.p_ladder(ion1,1,0,1) #destory operator on rock radial mode
...
ph_num1_1 = result1.expect[1] #axial com population
ph_num1_2 = result1.expect[2] #axial tilt population
ph_num1_3 = result1.expect[3] #radial tilt population
ph_num1_4 = result1.expect[4] #radial rock population
sigma_1 = result1.expect[5] #survival probablity  
#%%plot result    
...
p2.plot(tplot,ph_num1_1,'-',label=r'Axial COM')
p2.plot(tplot,ph_num1_2,'-',label=r'Axial Tilt')
...
ph_num2_1 = result2.expect[1] #axial com population
ph_num2_2 = result2.expect[2] #axial tilt population
...
p1.tick_params(axis='y', labelsize=13)
p2 = fig1.add_subplot(122)
p2.plot(tplot,ph_num2_1,'-',label=r'Axial COM')
p2.plot(tplot,ph_num2_2,'-',label=r'Axial Tilt')
